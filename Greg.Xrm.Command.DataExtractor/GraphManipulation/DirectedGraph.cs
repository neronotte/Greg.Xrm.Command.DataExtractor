...
namespace Greg.Xrm.Command.DataExtractor.GraphManipulation
{
		...
		where T : INodeContent
	{
		...
		{
			this.nodes = new Dictionary<object, DirectedNode<T>>();
		}
		...
		/// </summary>
		public bool HasNodes => this.nodes.Count > 0;
		...
		/// </summary>
		public int ArcCount => this.arcs.Count;
		...
		public DirectedGraph<T> AddNode(T node)
		{
			lock(this.syncRoot)
			{
				...
				this.nodes[node.Key] = new DirectedNode<T>(node);
			}
			return this;
		}
		...
		public DirectedGraph<T> AddArch(T from, T to, IReadOnlyDictionary<string, object>? additionalInfo = null)
		{
			lock(this.syncRoot)
			{
				if (!this.nodes.TryGetValue(from.Key, out var fromNode))
					throw new ArgumentException($"Node with key {from.Key} does not exists", nameof(from));
				if (!this.nodes.TryGetValue(to.Key, out var toNode))
		...
		{
			var clone = new DirectedGraph<T>(this.nodes.Values.Select(x => x.Content));
		...
		{
			return this.nodes.Values.Where(x => x.OutboundArcs.Count == 0).Select(x => x.Content).ToList();
		}
		...
		private void FindAllCycles(DirectedNode<T> node, HashSet<object> visitedNodes, List<DirectedArc<T>> path, List<Cycle<T>> allCycles)
		{
			...
			foreach (var outboundArc in node.OutboundArcs)
			{
				...
				if (!this.nodes.ContainsKey(nextNode.Content.Key))
				{
					continue;
				}
		...
		public DirectedGraph<T> RemoveNodes(IEnumerable<T> nodeContents)
		{
			...
			{
				var node = this.nodes[nodeContent.Key];
				...
				this.nodes.Remove(node.Content.Key);
			}
		...
		{
			return this.nodes.Values.Select(x => x.Content).GetEnumerator();
		}
